...
s=s.replace('status("APP","OK","R8 TCP 중계 유지 · NetShare SOOD reply-port proxy 추가 · 자체 VPN 없음");','status("APP","OK","R8 TCP 중계 유지 · PC Roon 원본 reply-port 보존 · 자체 VPN 없음");')

old='''                    if(!isSoodQuery(data))continue;\n                    if(isInjectedQuery(data))continue;\n                    String service=soodProp(data,"query_service_id");'''
new='''                    if(!isSoodQuery(data))continue;\n                    if(isInjectedQuery(data))continue;\n                    if(!PC_LAN.equals(p.getAddress().getHostAddress()))continue;\n                    String service=soodProp(data,"query_service_id");'''
...
new='byte[] query=data;workers.execute(()->forwardOriginalRoonQueryToRelay(src,srcPort,query));'
...
s=p.read_text(encoding='utf-8')
s=s.replace('기존 PHONE RoonLink/NetShare 그대로 · TCP 경로 유지 · Roon UDP reply-port proxy · v1.7','기존 PHONE RoonLink/NetShare 그대로 · PC Roon 원본 reply-port 보존 · v1.8')
...
s=p.read_text(encoding='utf-8').replace('versionCode = 8','versionCode = 9').replace('versionName = "1.7-netshare-sood-reply-proxy"','versionName = "1.8-origin-preserved-roon-query"')
p.write_text(s,encoding='utf-8')
print('S26 origin-preserved query patch applied')
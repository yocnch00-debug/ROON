...
public class MainActivity extends Activity {
    ...
    private View makeUi(){
        ...
        TextView sub=new TextView(this);sub.setText("기존 PHONE RoonLink/NetShare 그대로 · PC Relay는 물리 인터넷 직결 · 자체 VPN 없음");sub.setTextSize(14);sub.setPadding(0,dp(4),0,dp(14));root.addView(sub);
        ...
        LinearLayout addrRow=new LinearLayout(this);addrRow.setOrientation(LinearLayout.HORIZONTAL);addrRow.setPadding(0,dp(4),0,dp(10));
        ...
        addrRow.addView(pcHost,new LinearLayout.LayoutParams(0,dp(48),3f));
        addrRow.addView(pcPort,new LinearLayout.LayoutParams(0,dp(48),1.25f));
        ...
        Button restart=new Button(this);restart.setText("주소 저장 + 게이트웨이 다시 시작");restart.setOnClickListener(v->{saveAddress();stopService(new Intent(this,GatewayService.class));new Handler(Looper.getMainLooper()).postDelayed(this::startGateway,500);});root.addView(restart);
    ...
    private void saveAddress(){
        String host=pcHost==null?DEFAULT_PC_HOST:pcHost.getText().toString().trim();
        if(host.isEmpty())host=DEFAULT_PC_HOST;
        int port=DEFAULT_PC_PORT;
        try{port=Integer.parseInt(pcPort.getText().toString().trim());}catch(Throwable ignored){}
        if(port<1||port>65535)port=DEFAULT_PC_PORT;
        getSharedPreferences("gateway",MODE_PRIVATE).edit().putString("pc_host",host).putInt("pc_port",port).apply();

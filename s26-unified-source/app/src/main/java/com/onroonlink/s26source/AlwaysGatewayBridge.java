...
final class AlwaysGatewayBridge {
    ...
    private final AlwaysGatewayService service;
    ...
    AlwaysGatewayBridge(AlwaysGatewayService service) {
    ...
    void start() {
        synchronized (lock) {
            if (running) {
                status("LISTEN", "WAIT", "51921 listener 이미 실행중");
                return;
            }
            running = true;
    ...
    void stop() {
        ...
        status("LISTEN", "STOP", "51921 listener 중지");
    }
    ...
    private void serverLoop() {
        while (running) {
            ...
            try {
                ...
                server = ss;
                status("LISTEN", "WAIT", "0.0.0.0:" + LISTEN_PORT + " · R8 대기 · PHONE 상태와 독립");
                log("51921 listener OPEN");

...
public class ShareHostService extends Service {
    ...
    private synchronized void startSocks(InetAddress addr){ if(socks!=null&&socks.isRunning()&&addr.equals(socks.getBindAddress())) return; if(socks!=null)socks.close(); socks=new Socks5Server(addr,SOCKS_PORT,cellular::get,this::status); try{socks.start();}catch(Exception e){status("SOCKS 시작 실패: "+e.getMessage());} }

...
public class ShareHostService extends Service {
    ...
    private void ensureGroup(){ try{ p2p.requestGroupInfo(channel,g->{ if(g!=null&&g.isGroupOwner()){ publishGroupInfo(g); refreshConnection(); addService(); } else createGroup(); }); }catch(SecurityException e){status("권한 필요: "+e.getMessage());} }
    private void createGroup(){ try{ p2p.createGroup(channel,new WifiP2pManager.ActionListener(){ public void onSuccess(){ status("Wi-Fi Direct 그룹 생성됨 · SSID/비밀번호 확인 중"); addService(); new Handler(getMainLooper()).postDelayed(()->{refreshGroupInfo();refreshConnection();},600); } public void onFailure(int reason){ status("그룹 생성 대기/실패 code="+reason+" · 재확인"); new Handler(getMainLooper()).postDelayed(ShareHostService.this::ensureGroup,2500); }}); }catch(SecurityException e){status("Wi-Fi Direct 권한 필요");} }
    private void addService(){ try{ Map<String,String> txt=new HashMap<>(); txt.put("port",String.valueOf(SOCKS_PORT)); txt.put("version","0.2"); txt.put("mode","cellular"); WifiP2pDnsSdServiceInfo si=WifiP2pDnsSdServiceInfo.newInstance("ONShareLink","_onsharelink._tcp",txt); p2p.clearLocalServices(channel,new WifiP2pManager.ActionListener(){ public void onSuccess(){ p2p.addLocalService(channel,si,new WifiP2pManager.ActionListener(){public void onSuccess(){} public void onFailure(int r){}}); } public void onFailure(int r){} }); }catch(SecurityException ignored){} }
    private void refreshGroupInfo(){ try{p2p.requestGroupInfo(channel,this::publishGroupInfo);}catch(SecurityException ignored){} }
    private void publishGroupInfo(WifiP2pGroup g){ if(g==null||!g.isGroupOwner())return; String ssid=g.getNetworkName(); String pass=""; try{pass=g.getPassphrase();}catch(Throwable ignored){} int n=g.getClientList()==null?0:g.getClientList().size(); SharedPreferences.Editor e=getSharedPreferences("sharelink",0).edit().putInt("client_count",n); if(ssid!=null)e.putString("wifi_ssid",ssid); if(pass!=null)e.putString("wifi_password",pass); e.apply(); status("Wi-Fi 준비됨 · "+(ssid==null?"DIRECT 그룹":ssid)+" · 연결 기기 "+n+"대 · "+(cellular.get()!=null?"모바일데이터 OK":"모바일데이터 대기")); }

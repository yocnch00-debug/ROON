...
public class MainActivity extends Activity {
    ...
    private final BroadcastReceiver receiver=new BroadcastReceiver(){
        @Override public void onReceive(Context c,Intent i){
            ...
            bindingSwitch=true;shareSwitch.setChecked(en);bindingSwitch=false;refreshInfo();
        }
    };
    ...
    @Override public void onCreate(Bundle b){
        ...
        shareSwitch.setOnCheckedChangeListener((button,checked)->{
            if(bindingSwitch)return;requestPerms();getSharedPreferences("sharelink",0).edit().putBoolean("enabled",checked).apply();
            startHost(checked?ShareHostService.ACTION_START:ShareHostService.ACTION_STOP);refreshInfo();
        });
        apply.setOnClickListener(v->{
            String code=pairingInput.getText().toString().trim();if(!Pairing.setCode(this,code)){Toast.makeText(this,"숫자 8자리로 입력해 주세요",Toast.LENGTH_SHORT).show();return;}
            getSharedPreferences("sharelink",0).edit().putBoolean("enabled",true).apply();bindingSwitch=true;shareSwitch.setChecked(true);bindingSwitch=false;startHost(ShareHostService.ACTION_RECREATE);Toast.makeText(this,"새 비밀번호로 공유망을 다시 만듭니다",Toast.LENGTH_SHORT).show();
        });
        ...
        logs.setOnClickListener(v->{
            String t=HostDiag.tail(this);android.content.ClipboardManager cm=(android.content.ClipboardManager)getSystemService(CLIPBOARD_SERVICE);cm.setPrimaryClip(android.content.ClipData.newPlainText("ON ShareLink Host log",t));Toast.makeText(this,"S26 Host 진단 로그 복사됨",Toast.LENGTH_SHORT).show();
        });

        setContentView(sv);requestPerms();refreshInfo();
    }
    ...
    private void requestPerms(){
        ...
        if(Build.VERSION.SDK_INT>=33){if(checkSelfPermission(Manifest.permission.NEARBY_WIFI_DEVICES)!=PackageManager.PERMISSION_GRANTED)p.add(Manifest.permission.NEARBY_WIFI_DEVICES);if(checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED)p.add(Manifest.permission.POST_NOTIFICATIONS);}
        if(checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)!=PackageManager.PERMISSION_GRANTED)p.add(Manifest.permission.ACCESS_FINE_LOCATION);if(!p.isEmpty())requestPermissions(p.toArray(new String[0]),7);
    }

    private void startHost(String action){Intent i=new Intent(this,ShareHostService.class).setAction(action);if(Build.VERSION.SDK_INT>=26)startForegroundService(i);else startService(i);}
    @Override protected void onStart(){super.onStart();pairingInput.setText(Pairing.code(this));refreshInfo();IntentFilter f=new IntentFilter(ShareHostService.ACTION_STATUS);if(Build.VERSION.SDK_INT>=33)registerReceiver(receiver,f,Context.RECEIVER_NOT_EXPORTED);else registerReceiver(receiver,f);if(getSharedPreferences("sharelink",0).getBoolean("enabled",false))startHost(ShareHostService.ACTION_START);}

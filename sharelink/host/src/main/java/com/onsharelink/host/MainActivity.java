...
public class MainActivity extends Activity {
    ...
    private final BroadcastReceiver receiver = new BroadcastReceiver() {
        @Override public void onReceive(Context c, Intent i) {
            if (ShareHostService.ACTION_STATUS.equals(i.getAction())) status.setText(i.getStringExtra("text"));
        }
    };
    ...
    @Override protected void onStart(){ super.onStart(); IntentFilter f=new IntentFilter(ShareHostService.ACTION_STATUS); if(Build.VERSION.SDK_INT>=33) registerReceiver(receiver,f,Context.RECEIVER_NOT_EXPORTED); else registerReceiver(receiver,f); if(getSharedPreferences("sharelink",0).getBoolean("enabled",false)) startHost(); }

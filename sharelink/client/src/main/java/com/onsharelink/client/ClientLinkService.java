...
public class ClientLinkService extends Service {
    ...
    @Override public void onCreate(){
        ...
        foreground("ShareLink 저장 Wi-Fi 확인중");
        registerWifiCallback();
        Diag.log(this,"LINK_SERVICE_CREATE sdk="+Build.VERSION.SDK_INT);
    }
    ...
    @Override public int onStartCommand(Intent i,int f,int id){
        ...
        if(Build.VERSION.SDK_INT<30)ensureLegacySavedNetwork(code);
        status("1/4 ShareLink Wi-Fi 연결 대기 · "+SHARE_SSID);
        schedule(150);
        return START_STICKY;
    }
    ...
    private void foreground(String t){
        ...
        if(Build.VERSION.SDK_INT>=26)nm.createNotificationChannel(new NotificationChannel("sharelink_client","ON ShareLink Client",NotificationManager.IMPORTANCE_LOW));
        startForeground(5201,new Notification.Builder(this,Build.VERSION.SDK_INT>=26?"sharelink_client":null)
    ...
    private void status(String s){
        Diag.log(this,s);
        sendBroadcast(new Intent(ACTION_STATUS).setPackage(getPackageName()).putExtra("text",s));
        NotificationManager nm=(NotificationManager)getSystemService(NOTIFICATION_SERVICE);
        nm.notify(5201,new Notification.Builder(this,Build.VERSION.SDK_INT>=26?"sharelink_client":null)
                .setSmallIcon(android.R.drawable.stat_sys_download)
    ...
    @SuppressWarnings("deprecation") private void ensureLegacySavedNetwork(String code){
        ...
        try{
            WifiConfiguration c=new WifiConfiguration();c.SSID="\""+SHARE_SSID+"\"";c.preSharedKey="\""+code+"\"";
            int netId=wm.addNetwork(c);Diag.log(this,"LEGACY_ADD_NETWORK id="+netId);
            if(netId>=0){boolean sv=wm.saveConfiguration();boolean en=wm.enableNetwork(netId,true);boolean rc=wm.reconnect();Diag.log(this,"LEGACY_SAVE="+sv+" ENABLE="+en+" RECONNECT="+rc);}
        }catch(Exception e){Diag.log(this,"LEGACY_WIFI_ERROR "+e);}
    }
    ...
    private void checkNow(){
        ...
        if(candidates.isEmpty()){
            probing.set(false);
            stopService(new Intent(this,ShareVpnService.class));
            status("1/4 ShareLink Wi-Fi 연결 대기 · 저장된 "+SHARE_SSID+" 자동 접속 기다리는 중");
        ...
        io.execute(()->{
            ...
            for(Candidate c:candidates){
                ProbeResult r=probe(c.network,c.gateway,code);
                Diag.log(this,"SOCKS_PROBE net="+c.network+" host="+c.gateway+" result="+r);
    ...
    private List<Candidate> findShareWifiCandidates(){
        ...
        try{
            for(Network n:cm.getAllNetworks()){
                ...
                LinkProperties lp=cm.getLinkProperties(n);if(lp==null)continue;
                String local=null;
                for(LinkAddress la:lp.getLinkAddresses()){
                    InetAddress a=la.getAddress();
                    if(a instanceof Inet4Address&&a.getHostAddress().startsWith("192.168.49.")){local=a.getHostAddress();break;}
                }
                if(local==null)continue;
                Diag.log(this,"WIFI_CONNECTED net="+n+" if="+lp.getInterfaceName()+" ip="+local+" dns="+lp.getDnsServers());
                LinkedHashSet<String> gateways=new LinkedHashSet<>();
                gateways.add("192.168.49.1");
                for(RouteInfo r:lp.getRoutes()){
                    InetAddress x=r.getGateway();
                    if(x instanceof Inet4Address&&!x.isAnyLocalAddress()&&x.getHostAddress().startsWith("192.168.49."))gateways.add(x.getHostAddress());
                }
    ...
    private ProbeResult probe(Network n,String host,String code){
        ...
        try(Socket s=n.getSocketFactory().createSocket()){
            ...
            s.connect(new InetSocketAddress(host,51950),3000);
            s.setSoTimeout(6000);
            InputStream in=s.getInputStream();OutputStream out=s.getOutputStream();
            out.write(new byte[]{5,1,2});out.flush();
            byte[] hello=readN(in,2);if((hello[0]&255)!=5||(hello[1]&255)!=2)return ProbeResult.NO_HOST;
            byte[] u="onshare".getBytes(StandardCharsets.UTF_8),p=code.getBytes(StandardCharsets.UTF_8);
            ByteArrayOutputStream a=new ByteArrayOutputStream();a.write(1);a.write(u.length);a.write(u);a.write(p.length);a.write(p);
            out.write(a.toByteArray());out.flush();
            byte[] auth=readN(in,2);if((auth[0]&255)!=1||(auth[1]&255)!=0)return ProbeResult.BAD_CODE;
            out.write(new byte[]{5,1,0,1,1,1,1,1,1,(byte)0xbb});out.flush();
            byte[] rh=readN(in,4);if((rh[0]&255)!=5)return ProbeResult.NO_HOST;
            int rep=rh[1]&255;
            int atyp=rh[3]&255;
            if(atyp==1)readN(in,4);else if(atyp==4)readN(in,16);else if(atyp==3)readN(in,readU8(in));else return ProbeResult.NO_HOST;
            readN(in,2);
            if(rep==0)return ProbeResult.OK;
            if(rep==3||rep==4||rep==5)return ProbeResult.NO_CELLULAR;
            return ProbeResult.NO_HOST;
    ...
    private void handleSuccess(String host){
        misses=0;
        getSharedPreferences("sharelink",0).edit().putString("host_ip",host).apply();
        status("4/4 S26 Wi-Fi + LTE/5G 실제 확인 완료 · 인터넷 VPN 시작");
    ...
    private void handleFailure(String text){
        misses++;status(text);
        if(misses>=2)stopService(new Intent(this,ShareVpnService.class));
    }

    private void startVpn(String host){
        Diag.log(this,"VPN_START_REQUEST host="+host);
        Intent i=new Intent(this,ShareVpnService.class).putExtra("host",host);
    ...
    @Override public void onDestroy(){
        Diag.log(this,"LINK_SERVICE_DESTROY");
        h.removeCallbacksAndMessages(null);io.shutdownNow();
        try{if(wifiCallback!=null)cm.unregisterNetworkCallback(wifiCallback);}catch(Exception ignored){}
        super.onDestroy();
    }

...
public class ClientLinkService extends Service {
    ...
    private void handleSuccess(String host){misses=0;activeHost=host;getSharedPreferences("sharelink",0).edit().putString("host_ip",host).apply();status("S26 DIRECT Wi-Fi + 모바일 데이터 확인됨 · "+host+" · VPN 준비");if(VpnService.prepare(this)==null)startVpn(host);else sendBroadcast(new Intent(ACTION_NEED_VPN).setPackage(getPackageName()));}

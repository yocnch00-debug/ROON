...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        TextView d=new TextView(this);d.setText("최초 1회 ShareLink Wi-Fi를 Android에 '저장된 네트워크'로 등록합니다.\n이후 S26 공유망이 켜지면 자동 Wi-Fi → S26 LTE/5G 확인 → VPN까지 이어집니다.\n\n로컬 192.168.x.x / RoonLink / SMB는 VPN 밖으로 보호합니다.");d.setTextSize(16);d.setPadding(0,14,0,18);root.addView(d);
        ...
        connectSwitch.setOnCheckedChangeListener((button,checked)->{
            ...
            if(checked){
                ...
                saveCode(code,true);
                requestPerms();
                beginProvisionOrLink(code,false);
            }else{
                getSharedPreferences("sharelink",0).edit().putBoolean("enabled",false).apply();
                stopService(new Intent(this,ClientLinkService.class));
                stopService(new Intent(this,ShareVpnService.class));
                status.setText("자동 연결 OFF");
                Diag.log(this,"USER_SWITCH_OFF");
            }
        });
        ...
        provision.setOnClickListener(v->{
            ...
            saveCode(code,true);
            bindingSwitch=true;connectSwitch.setChecked(true);bindingSwitch=false;
            cleanupOldSuggestions(code);
            launchWifiSave(code);
        });

        logs.setOnClickListener(v->{
            String t=Diag.tail(this);
            android.content.ClipboardManager cm=(android.content.ClipboardManager)getSystemService(CLIPBOARD_SERVICE);
            cm.setPrimaryClip(android.content.ClipData.newPlainText("ON ShareLink log",t));
    ...
    private void migrateV1(){
        ...
        if(p.getBoolean("v1_migrated",false))return;
        String code=p.getString("pairing_code","");
        cleanupOldSuggestions(code);
        p.edit().remove("suggested_code").remove("wifi_saved_code").putBoolean("v1_migrated",true).apply();
        Diag.log(this,"V1_MIGRATION cleared legacy suggestion state");
    }
    ...
    private void cleanupOldSuggestions(String code){
        ...
        try{
            ...
            for(String c:codes){
                WifiNetworkSuggestion s=new WifiNetworkSuggestion.Builder().setSsid(ClientLinkService.SHARE_SSID).setWpa2Passphrase(c).build();
                int r=wm.removeNetworkSuggestions(Collections.singletonList(s));
                Diag.log(this,"REMOVE_OLD_SUGGESTION code="+r);
            }
        }catch(Exception e){Diag.log(this,"REMOVE_OLD_SUGGESTION_ERROR "+e);}
    }

    private void saveCode(String code,boolean enabled){
        getSharedPreferences("sharelink",0).edit().putString("pairing_code",code).putBoolean("enabled",enabled).apply();
        Diag.log(this,"USER_CODE_SET enabled="+enabled);
    }

    private void beginProvisionOrLink(String code,boolean forceProvision){
        SharedPreferences p=getSharedPreferences("sharelink",0);
        String saved=p.getString("wifi_v1_saved_code","");
        if(Build.VERSION.SDK_INT>=30&&(forceProvision||!code.equals(saved))){cleanupOldSuggestions(code);launchWifiSave(code);return;}
        startLink();
    }
    ...
    private void launchWifiSave(String code){
        try{
            ...
            status.setText("최초 1회 · Android 화면에서 "+ClientLinkService.SHARE_SSID+" 저장을 눌러 주세요");
            Diag.log(this,"WIFI_SAVE_UI_REQUEST ssid="+ClientLinkService.SHARE_SSID);
            startActivityForResult(x,WIFI_SAVE_REQ);
        }catch(Exception e){
            status.setText("Wi-Fi 저장 화면 실행 실패 · "+e.getClass().getSimpleName());
            Diag.log(this,"WIFI_SAVE_UI_ERROR "+e);
        }
    }
    ...
    private void startLink(){
        Intent i=new Intent(this,ClientLinkService.class).setAction(ClientLinkService.ACTION_START);
        if(Build.VERSION.SDK_INT>=26)startForegroundService(i);else startService(i);
    }

    private void askVpn(){
        Intent p=VpnService.prepare(this);
        if(p==null)startVpnFromSavedHost();else startActivityForResult(p,VPN_REQ);
    }

    private void startVpnFromSavedHost(){
        String h=getSharedPreferences("sharelink",0).getString("host_ip",null);if(h==null)return;
        Intent i=new Intent(this,ShareVpnService.class).putExtra("host",h);
        if(Build.VERSION.SDK_INT>=26)startForegroundService(i);else startService(i);
    }
    ...
    @Override protected void onActivityResult(int r,int c,Intent d){
        ...
        if(r==VPN_REQ){
            if(c==RESULT_OK){Diag.log(this,"VPN_PERMISSION_OK");startVpnFromSavedHost();}
            else{Diag.log(this,"VPN_PERMISSION_DENIED");status.setText("VPN 허용이 필요합니다");}
            return;
        }
        if(r==WIFI_SAVE_REQ){
            if(c==RESULT_OK){
                String code=getSharedPreferences("sharelink",0).getString("pairing_code","");
                getSharedPreferences("sharelink",0).edit().putString("wifi_v1_saved_code",code).apply();
                status.setText("ShareLink Wi-Fi 저장 완료 · 이제 자동 접속 대기");
                Diag.log(this,"WIFI_SAVE_RESULT_OK");
                startLink();
            }else{
                status.setText("Wi-Fi 저장 취소됨 · 다시 등록 버튼을 눌러 주세요");
                Diag.log(this,"WIFI_SAVE_RESULT_CANCEL result="+c);
            }
        }
    }

    private void refreshSwitch(){
        boolean en=getSharedPreferences("sharelink",0).getBoolean("enabled",false);
        bindingSwitch=true;connectSwitch.setChecked(en);bindingSwitch=false;
    }

    @Override protected void onStart(){
        super.onStart();
        IntentFilter f=new IntentFilter();f.addAction(ClientLinkService.ACTION_STATUS);f.addAction(ClientLinkService.ACTION_NEED_VPN);f.addAction(ShareVpnService.ACTION_STATUS);
        ...
        if(p.getBoolean("enabled",false)&&code.matches("\\d{8}")&&(Build.VERSION.SDK_INT<30||code.equals(p.getString("wifi_v1_saved_code",""))))startLink();
    }

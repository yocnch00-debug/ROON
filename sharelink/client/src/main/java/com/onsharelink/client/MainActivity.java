...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b); LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(36,48,36,36); TextView title=new TextView(this);title.setText("ON ShareLink Client v0.2");title.setTextSize(26);root.addView(title); TextView d=new TextView(this);d.setText("1) S26 Host에 표시된 DIRECT Wi-Fi 이름/비밀번호로 먼저 일반 Wi-Fi 연결\n2) 아래에 S26 앱 페어링 코드 입력\n3) 자동 연결 시작\n\n인터넷만 S26 데이터로 보내고 로컬/Roon/SMB는 Wi-Fi 직통 유지");d.setTextSize(16);d.setPadding(0,18,0,18);root.addView(d); pairing=new EditText(this);pairing.setHint("S26 앱 페어링 코드 숫자 8자리");pairing.setInputType(InputType.TYPE_CLASS_NUMBER);pairing.setText(getSharedPreferences("sharelink",0).getString("pairing_code",""));root.addView(pairing);Button wifi=new Button(this);wifi.setText("Wi-Fi 설정 열기");root.addView(wifi);status=new TextView(this);status.setText("S26의 DIRECT Wi-Fi에 먼저 연결해 주세요");status.setTextSize(17);status.setPadding(0,14,0,26);root.addView(status); Button start=new Button(this);start.setText("자동 연결 시작");root.addView(start);Button stop=new Button(this);stop.setText("연결 중지");root.addView(stop);wifi.setOnClickListener(v->startActivity(new Intent(Settings.ACTION_WIFI_SETTINGS)));start.setOnClickListener(v->{String code=pairing.getText().toString().trim();if(!code.matches("\\d{8}")){Toast.makeText(this,"숫자 8자리 페어링 코드를 입력해 주세요",Toast.LENGTH_SHORT).show();return;}getSharedPreferences("sharelink",0).edit().putString("pairing_code",code).putBoolean("enabled",true).apply();requestPerms();startLink();});stop.setOnClickListener(v->{getSharedPreferences("sharelink",0).edit().putBoolean("enabled",false).apply();stopService(new Intent(this,ClientLinkService.class));stopService(new Intent(this,ShareVpnService.class));status.setText("중지됨");});setContentView(root);requestPerms();}

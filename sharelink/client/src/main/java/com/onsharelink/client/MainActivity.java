...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b); LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(36,48,36,36); TextView title=new TextView(this);title.setText("ON ShareLink Client");title.setTextSize(26);root.addView(title); TextView d=new TextView(this);d.setText("R8 II와 일반 Android 폰 공용 클라이언트\n인터넷만 S26 모바일 데이터로 전송 · 로컬/Roon/SMB는 Wi-Fi 직통");d.setTextSize(16);d.setPadding(0,18,0,18);root.addView(d); pairing=new EditText(this);pairing.setHint("S26에 표시된 8자리 페어링 코드");pairing.setInputType(InputType.TYPE_CLASS_NUMBER);pairing.setText(getSharedPreferences("sharelink",0).getString("pairing_code",""));root.addView(pairing);status=new TextView(this);status.setText("대기중");status.setTextSize(17);status.setPadding(0,14,0,26);root.addView(status); Button start=new Button(this);start.setText("자동 연결 시작");root.addView(start);Button stop=new Button(this);stop.setText("연결 중지");root.addView(stop);start.setOnClickListener(v->{String code=pairing.getText().toString().trim();if(code.length()!=8){Toast.makeText(this,"8자리 페어링 코드를 입력해 주세요",Toast.LENGTH_SHORT).show();return;}getSharedPreferences("sharelink",0).edit().putString("pairing_code",code).putBoolean("enabled",true).apply();requestPerms();startLink();});stop.setOnClickListener(v->{getSharedPreferences("sharelink",0).edit().putBoolean("enabled",false).apply();stopService(new Intent(this,ClientLinkService.class));stopService(new Intent(this,ShareVpnService.class));status.setText("중지됨");});setContentView(root);requestPerms();}

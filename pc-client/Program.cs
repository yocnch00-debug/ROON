...
internal sealed class MainForm : Form
{
    ...
    private int tunnelIfIndex = -1;
    private bool closing;

    // Same public-only IPv4 split used by the proven Android ShareLink client.
    // RFC1918, link-local, multicast and the S26 192.168.49.0/24 path stay outside the TUN.
    ...
    public MainForm()
    {
        ...
        Append("PC Client 준비 완료 · S26 Host / R8 골든판은 변경하지 않습니다.");
    }
    ...
    private async Task StartClicked()
    {
        ...
        try
        {
            ...
            Append("ShareLink 연결 완료 · TCP/UDP 공인 IPv4가 S26으로 전달됩니다.");
            StartMonitor(code);
        }
        catch (Exception ex)
        {
    ...
    private async Task StopClicked()
    {
        ...
        SetDisconnectedUi();
        SetStatus("연결 해제됨");
        Append("ShareLink 터널과 전용 경로를 정리했습니다.");
    }
    ...
    private void StartCore(string code)
    {
        ...
        if (!HasS26Link())
        {
            if (wifiAuto.Checked)
            {
                ...
                Append("Windows Wi-Fi 프로필 등록/연결 시도");
                ProvisionWifi(code);
            }
        ...
        ExtractEmbedded("ONShareLink.msys-2.0.dll", Path.Combine(workDir, "msys-2.0.dll"));
        string cfg = WriteTunnelConfig(code);

        SetStatus("3/4 Windows TUN 시작중");
        StartTunnel(cfg);
        tunnelIfIndex = WaitForTunAdapter(TimeSpan.FromSeconds(15));
        Append($"Wintun 준비됨 · IF={tunnelIfIndex}");

        ConfigureTunDns();
        InstallRoutes(tunnelIfIndex);
    ...
    private void StopCore()
    {
        try
        {
            if (tunnelIfIndex > 0) RemoveRoutes(tunnelIfIndex);
        }
        catch { }
    ...
    private void StartTunnel(string configPath)
    {
        ...
        tunnelProcess.OutputDataReceived += (_, e) =>
        {
            if (!string.IsNullOrWhiteSpace(e.Data)) Append("HEV " + e.Data);
        };
        tunnelProcess.ErrorDataReceived += (_, e) =>
        {
            if (!string.IsNullOrWhiteSpace(e.Data)) Append("HEV " + e.Data);
        };
        ...
        foreach (NetworkInterface ni in NetworkInterface.GetAllNetworkInterfaces())
        {
            ...
            foreach (UnicastIPAddressInformation u in ni.GetIPProperties().UnicastAddresses)
            {
                if (u.Address.AddressFamily == AddressFamily.InterNetwork &&
                    u.Address.ToString().StartsWith(prefix, StringComparison.Ordinal))
    ...
    private static int WaitForTunAdapter(TimeSpan timeout)
    {
        ...
        while (sw.Elapsed < timeout)
        {
            foreach (NetworkInterface ni in NetworkInterface.GetAllNetworkInterfaces())
            {
                bool hasTunIp = ni.GetIPProperties().UnicastAddresses.Any(x =>
                    x.Address.AddressFamily == AddressFamily.InterNetwork && x.Address.ToString() == TunIp);
                if (!hasTunIp) continue;
    ...
    private static void ProbeSocks(string code, int timeoutMs)
    {
        ...
        if (!wh.WaitOne(timeoutMs)) throw new IOException("S26 ShareLink 서버에 연결할 수 없습니다.");
    ...
    private static byte[] ReadN(Stream stream, int count)
    {
    ...
    private void InstallRoutes(int ifIndex)
    {
        ...
        {
            (string network, string mask) = CidrToRoute(cidr);
            // HEV's Windows guidance uses 0.0.0.0 as the on-link next hop for the Wintun interface.
            int exitCode = RunHidden("route.exe", $"ADD {network} MASK {mask} 0.0.0.0 METRIC 5 IF {ifIndex}", false);
            if (exitCode != 0)
            {
                RemoveRoutes(ifIndex);
                throw new IOException("Windows 공인 IPv4 경로 생성 실패: " + cidr);
            }
        }
        Append($"공인 IPv4 split routes {PublicRoutes.Length}개 적용");
    }

    private void RemoveRoutes(int ifIndex)
    {
        foreach (string cidr in PublicRoutes)
        {
            (string network, string mask) = CidrToRoute(cidr);
            RunHidden("route.exe", $"DELETE {network} MASK {mask} 0.0.0.0 IF {ifIndex}", false);
        }
    }
    ...
    private static (string network, string mask) CidrToRoute(string cidr)
    {
        ...
        string dotted = $"{(mask >> 24) & 255}.{(mask >> 16) & 255}.{(mask >> 8) & 255}.{mask & 255}";
        return (parts[0], dotted);
    }

    private void KillOldTunnel()
    {
        string wanted = Path.Combine(workDir, "hev-socks5-tunnel.exe");
        try
        {
            foreach (Process p in Process.GetProcessesByName("hev-socks5-tunnel"))
            {
                try
                {
                    string? path = p.MainModule?.FileName;
                    if (!string.Equals(path, wanted, StringComparison.OrdinalIgnoreCase)) continue;
    ...
    private static int RunHidden(string file, string args, bool throwOnError)
    {
        ...
        if (!p.WaitForExit(15000))
        {
            ...
            if (throwOnError) throw new IOException(file + " 실행 시간 초과");
            return -1;
        }
        if (throwOnError && p.ExitCode != 0)
            throw new IOException($"{file} 실패({p.ExitCode}) {stdout} {stderr}");
        return p.ExitCode;
    }
    ...
    private static void ExtractEmbedded(string resourceName, string outputPath)
    {
        using Stream src = Assembly.GetExecutingAssembly().GetManifestResourceStream(resourceName)
            ?? throw new IOException("내장 엔진 누락: " + resourceName);
    ...
    private void StartMonitor(string code)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            while (!token.IsCancellationRequested && !closing)
            {
                ...
                try
                {
                    if (tunnelProcess == null || tunnelProcess.HasExited || !HasS26Link())
                        throw new IOException("link lost");
                    ProbeSocks(code, 3000);
                    misses = 0;
                }
                catch
                {
                    ...
                    if (misses >= 3)
                    {
                        SetStatus("연결 이상 감지 · 연결 해제 후 다시 연결해 주세요");

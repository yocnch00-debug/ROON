...
final class TunnelMux {
    ...
    TunnelMux(Socket s) throws IOException { in=new DataInputStream(new BufferedInputStream(s.getInputStream())); out=new DataOutputStream(new BufferedOutputStream(s.getOutputStream())); }

    synchronized void send(int type,int streamId,byte[] payload) throws IOException {
        if(payload==null)payload=new byte[0]; if(payload.length>MAX)throw new IOException("frame too large");
    ...
    static byte[] endpointPacket(String ip,int port,byte[] packet) throws IOException {
        byte[] ib=ip.getBytes(StandardCharsets.UTF_8); if(ib.length>255)throw new IOException("ip too long");

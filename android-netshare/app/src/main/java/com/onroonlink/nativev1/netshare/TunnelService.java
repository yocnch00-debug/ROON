...
public class TunnelService extends VpnService {
    ...
    static final String CH = "onrl_r8_v30";
    ...
    @Override public void onCreate() {
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            NotificationChannel c = new NotificationChannel(CH, "ON RoonLink R8 v30", NotificationManager.IMPORTANCE_LOW);
    ...
    @Override public int onStartCommand(Intent i, int flags, int id) {
        ...
        setStatus("v30 시작 · NetShare 물리 Wi-Fi 확인중");
        start(password, proxyHost, proxyPort);
        return START_STICKY;
    }

    Notification notification(String text) {
        return new Notification.Builder(this, CH)
                .setContentTitle("ON RoonLink R8 II v30")
                .setContentText(text)
    ...
    void start(String password, String proxyHost, int proxyPort) {
        stopTunnel();
        running.set(true);
        int owner = generation.incrementAndGet();
        main = new Thread(() -> runTunnel(password, proxyHost, proxyPort, owner), "ONRL-v30-main");
        main.start();
    }

    void runTunnel(String password, String proxyHost, int proxyPort, int owner) {
        long wait = 1000;
        while (running.get() && generation.get() == owner) {
            int gen = owner;
            try {
                if (!PairInfo.validPassword(password)) throw new IOException("비밀번호는 숫자 4~8자리여야 합니다");
                connectOnce(password, proxyHost, proxyPort, gen);
                wait = 1000;
            } catch (Exception e) {
                if (running.get() && generation.get() == gen) {
                    setStatusFor(gen, "재연결 · " + shortMessage(e));
                    closeCurrent(gen);
                }
                if (!running.get() || generation.get() != gen) break;
                try { Thread.sleep(wait); } catch (InterruptedException ignored) { break; }
    ...
    void connectOnce(String password, String proxyHost, int proxyPort, int gen) throws Exception {
        Network wifi = findPhysicalWifi();
        if (wifi != null) setStatusFor(gen, "NetShare 물리 Wi-Fi 확인 · " + proxyHost + ":" + proxyPort);
        else setStatusFor(gen, "물리 Wi-Fi 식별 실패 · 기본 경로로 8282 시도");

        setStatusFor(gen, "NetShare 8282 → PC TCP 51900 CONNECT 시도");
        SSLSocket sessionSock = connectViaHttpProxy(proxyHost, proxyPort, DIRECT_HOST, DIRECT_PORT, SERVER_FP, wifi, gen);
        synchronized (ioLock) {
            if (!running.get() || generation.get() != gen) {
        ...
        setStatusFor(gen, "PC TLS 연결됨 · DAP 인증중");
        sessionSock.setSoTimeout(30000);
        out.write(("ONRL1 " + password + " DAP\n").getBytes("UTF-8"));
        out.flush();
        String res = readUtf8Line(net);
        if (res == null) throw new IOException("PC 인증 응답 없음");
        if (!res.startsWith("OK ")) throw new IOException("PC 인증 실패: " + res);
        String ip = res.substring(3).trim();
        sessionSock.setSoTimeout(0);
        setStatusFor(gen, "PC 인증 성공 · " + ip + " · 단일 VPN 생성중");

        Builder b = new Builder()
                .setSession("ON RoonLink R8 II v30")
                .setMtu(1280)
                .addAddress(ip,24)
                .addRoute("0.0.0.0",0)
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            b.setBlocking(true);
            try { b.setMetered(false); } catch(Exception ignored) {}
        }
        ...
        setStatusFor(gen, "연결됨 · NetShare 외부경로 + PC Full Tunnel · Roon/인터넷");
        ...
        final SSLSocket ownedSock = sessionSock;
        final ParcelFileDescriptor ownedTun = sessionTun;
        final OutputStream ownedOut = out;
        ...
        Thread up = new Thread(() -> {
            ...
            try {
                while (running.get() && generation.get() == gen) {
                    int n = ti.read(buf);
                    if (n < 0) break;
                    writeFrame(ownedOut, buf, n);
                }
            } catch (Exception e) {
                setStatusFor(gen, "업로드 터널 종료 · " + shortMessage(e));
            } finally {
                try { ownedSock.close(); } catch(Exception ignored) {}
                try { ownedTun.close(); } catch(Exception ignored) {}
            }
        }, "ONRL-v30-up");
        up.start();
        ...
        try {
            ...
            throw new IOException("다운로드 터널 종료 · " + shortMessage(e), e);
        } finally {
            try { sessionSock.close(); } catch(Exception ignored) {}
            try { sessionTun.close(); } catch(Exception ignored) {}
            synchronized (ioLock) {
    ...
    SSLSocket connectViaHttpProxy(String proxyHost, int proxyPort, String targetHost, int targetPort, String fp, Network wifi, int gen) throws Exception {
        ...
        if (!protect(plain)) throw new IOException("외부 소켓 VPN 보호 실패");
        plain.setKeepAlive(true);
        plain.setTcpNoDelay(true);
        plain.connect(new InetSocketAddress(proxyHost, proxyPort), 10000);
        plain.setSoTimeout(30000);
        setStatusFor(gen, (bound ? "물리 Wi-Fi 고정" : "기본 Wi-Fi") + " · NetShare 8282 접속 · CONNECT 51900");
        ...
        if (status == null || status.isEmpty()) {
            ...
            throw new IOException("NetShare CONNECT 응답 없음");
        }
        ...
        if (code != 200) {
            try { plain.close(); } catch(Exception ignored) {}
            throw new IOException("NetShare CONNECT HTTP " + code);
        }
